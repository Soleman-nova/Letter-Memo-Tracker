...
export default function MultiSelect({ options, value, onChange, placeholder = 'Select...', className = '' }) {
  ...
  return (
    <div className={`relative ${className}`} ref={ref}>
      <button type="button" className="border rounded p-2 w-full text-left bg-white" onClick={() => setOpen(o => !o)}>
        {selectedLabels.length ? (
          <div className="flex flex-wrap gap-1">
            {selectedLabels.map((lbl, idx) => (
              <span key={idx} className="inline-flex items-center rounded bg-slate-200 px-2 py-0.5 text-xs">{lbl}</span>
            ))}
          </div>
        ) : (
      ...
      {open && (
        <div className="absolute z-20 mt-1 w-full max-h-60 overflow-auto rounded border bg-white shadow">
          {(options || []).filter(opt => opt && opt.value !== undefined).map((opt, idx) => {
            ...
            return (
              <label key={`${opt.value}-${idx}`} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 cursor-pointer select-none">
                <input type="checkbox" className="accent-blue-600" checked={checked} onChange={() => toggleValue(opt.value)} />
